...
def process_data(price, now):
    ...
    global high_point, float_width, low_point, high_buy, high_sell, low_buy, low_sell
    global have_position, position_status, high_sell_status, low_buy_status, max_x, max_time
    global high_sell_half, low_buy_half
    # 更新价格
    if price > high_point:
        high_point = price
        low_point = high_point - float_width
    ...
    # 有仓位情况，分别讨论四种仓位种类*****************************************************************
        ...
        if position_status == 'high_buy':
            ...
            elif high_sell_half == 1 and price < low_buy:
                # 止盈半仓
                high_sell_half = 0
                loss_x = loss_x / 2
                # 检测下单数量的合法性
            ...
            elif low_buy_status == 1 and price > low_buy:
                # 破新低并到低多点位
                low_buy_half = 1
                loss_x = 0
                if loss_x > max_x:
            ...
            if price == low_point:
            ...
            elif low_buy_half == 1 and price > high_sell:
                # 破新高并到高空点位
                low_buy_half = 0
                loss_x = loss_x / 2
                if loss_x > max_x:
                ...
                print('   目前亏损', loss_x, '时间', now)
                print('   持仓', all_amounts, '类型', position_status, '价格', price)
            elif high_sell_status == 1 and price < high_sell:
                # 破新高并到高空点位
                high_sell_half = 1
                loss_x = 0
                if loss_x > max_x:
