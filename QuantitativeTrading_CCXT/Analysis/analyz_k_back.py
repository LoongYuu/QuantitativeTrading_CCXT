...
symbol = 'ETHUSDT'  # 要交易的币种************
tra_amounts = 0.1  # 底仓数量*************
float_price = 30  # 策略的加仓或平仓价格**********
min_amount = 0.1  # 此币最小下单数量************
...
def process_data(price, now):
    ...
    global high_point, float_width, low_point, high_buy, high_sell, low_buy, low_sell, take_profit, stop_lose, back_board, old_point
    global have_position, position_status, high_sell_status, low_buy_status, high_sell_commit, low_buy_commit
    ...
    if have_position == 0:
        # 高空条件确认
        if price < high_sell and high_sell_status == 1 and high_sell_commit == 0:
            high_sell_commit = 1
            old_point = high_point
            print('突破新高后回踩到位,可以做高空单    时间', now, '点位', high_point - 100)
        elif price > low_buy and low_buy_status == 1 and low_buy_commit == 0:
            low_buy_commit = 1
            old_point = low_point
            print('突破新低后回踩到位,可以做低多单    时间', now, '点位', low_point + 100)
        # 做高空单
        if price > high_point - back_board and high_sell_commit == 1:
            # 开过单后重置状态
            high_sell_status = 0
            high_sell_commit = 0
        ...
        # 做低多单
        elif price < low_point + back_board and low_buy_commit == 1:
            # 开过单后重置状态
            low_buy_status = 0
            low_buy_commit = 0
    ...
    # 有仓位情况，分别讨论四种仓位种类*****************************************************************
        ...
        if position_status == 'high_sell':
            if price > open_position_price + stop_lose:
                # 打损高空单
            ...
            elif price < open_position_price - take_profit:
                # 直接止盈
