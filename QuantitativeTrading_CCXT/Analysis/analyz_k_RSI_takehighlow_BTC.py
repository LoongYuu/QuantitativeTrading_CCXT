...
win_price = 0
lose_price = 0
high_line = 70  # 上RSI做单边界
low_line = 30  # 下RSI做单边界
give_up_lose = 20000  # 认亏张数
win_rate = 3  # 止盈倍率
lose_rate = 1  # 止损倍率
continue_lose = 0  # 连输次数
max_continue_lose = 0  # 最大连输次数
...
def check_stop_take(highPrice, lowPrice, side, time):
    ...
    global lose_time, win_time, give_up_lose, continue_lose, max_continue_lose
    if side == 'Buy':
        if lowPrice < lose_price:
            ...
            have_position = 0
            loss_x = loss_x + qty
            if loss_x > give_up_lose:
                loss_x = 0
            print('止损,时间', time)
            lose_time = lose_time + 1
            continue_lose = continue_lose + 1
            if continue_lose > max_continue_lose:
                max_continue_lose = continue_lose
...
while time202561 < end_time:
    ...
    for kdata in reversed(kline):
        ...
        high_queue.append(highPrice)
        low_queue.append(lowPrice)
        if i > 0:
            i = i - 1
            continue
        ...
        if have_position == 1:
            ...
            if RSI12 >= high_line:
                ready_to_sell = 1
            elif ready_to_sell == 1 and RSI12 < high_line:
                ready_to_sell = 0
                # 设置合理止损
                price_width = max(high_queue[len(high_queue) - 12:]) - price
                # 过小区间不做
                if price_width < 300:
                    break
                qty = 0.001 + loss_x
                ...
                have_position = 1
                tra_side = 'Sell'
                win_price = price - price_width * win_rate
                lose_price = price + (price_width * lose_rate)
                print('开空⬇', qty, '总仓位', all_amounts, '价格', price, '时间', ktime)
                print('止损价格', lose_price)
            ...
            if RSI12 <= low_line:
                ready_to_buy = 1
            elif ready_to_buy == 1 and RSI12 > low_line:
                ready_to_buy = 0
                # 设置合理止损
                price_width = price - min(low_queue[len(low_queue) - 12:])
                # 过小区间不做
                if price_width < 300:
                    break
                qty = 0.001 + loss_x
...
print('总止盈次数', win_time, '总止损次数', lose_time, '胜率', win_time / (win_time + lose_time))
print('最大连续止损次数', max_continue_lose)
